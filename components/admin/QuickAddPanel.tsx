...
export default function QuickAddPanel({ isOpen, onClose, onRefresh }: QuickAddPanelProps) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      fd.append('accent_hex', '#7c3aed')
      fd.append('smoke_hex_scroll', '#7c3aed')
      fd.append('smoke_hex_preview', '#7c3aed')
          ...
          >
            ...
            <div className="flex gap-2 rounded-2xl bg-black/40 p-1 mb-8 border border-white/[0.03]">
              <button
                ...
                className={`flex-1 rounded-xl py-2 text-[10px] font-black uppercase tracking-widest transition ${activeTab === 'create' ? 'bg-violet-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
              >
                Fast Add
              </button>
              <button
                onClick={() => setActiveTab('stock')}
                className={`flex-1 rounded-xl py-2 text-[10px] font-black uppercase tracking-widest transition ${activeTab === 'stock' ? 'bg-violet-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
              >
                Stock Fix
              </button>
            </div>
            ...
            {activeTab === 'create' ? (
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="pt-4">
                   <button
                     disabled={busy}
                     className="w-full rounded-full bg-violet-600 py-4 text-xs font-black uppercase tracking-widest text-white shadow-xl hover:bg-violet-500 transition active:scale-[0.98] disabled:opacity-50"
                   >
                     {busy ? 'INJECTING...' : 'DIRECT INJECTION'}
                   </button>
                   <p className="mt-4 text-[9px] text-center text-slate-500 uppercase font-bold">
                     Note: Product will use default matte theme. <br/> Visit Inventory to add images and smoke FX.
                   </p>
                </div>
              </form>
            ) : (
